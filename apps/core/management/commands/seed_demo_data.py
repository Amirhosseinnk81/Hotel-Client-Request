...
# so the demo has someone who can assign tickets out of the box. After the
# accounts 0004 migration every existing operator is a regular one, and
# only a supervisor may assign.
SUPERVISORS = [
    ("sup_housekeeping", "ZZzz123!@#", "HOUSEKEEPING"),
    ("sup_frontdesk", "ZZzz123!@#", "FRONT_DESK"),
    ...
    def _seed_users(self, departments, reset_passwords):
        ...
        for username, password, dept_code in OPERATORS:
            ...
            if created or reset_passwords:
                ...
                operator.is_supervisor = False
                operator.save()
            self._log(created, "Operator", f"{operator.username} / {password} ({dept_code})")

        for username, password, dept_code in SUPERVISORS:
            ...
            if created or reset_passwords:
                ...
                supervisor.role = User.Role.OPERATOR
                supervisor.department = departments[dept_code]
                supervisor.is_supervisor = True
                supervisor.save()
            self._log(created, "Supervisor", f"{supervisor.username} / {password} ({dept_code})")
