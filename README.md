...
- ورود سریع با اسکن QR اتاق (شماره اتاق از پیش پر می‌شود)
- حالت تیره
...
برای هر اتاق یک QR بسازید که به این آدرس اشاره کند:

```
https://<host>/guest/login?room=305
```

اسکن آن، فرم ورود مهمان را با شماره اتاق از پیش پرشده باز می‌کند و پس از ورود موفق، مستقیم به فرم ثبت درخواست می‌رود (نه داشبورد) — چون مهمانی که QR داخل اتاق را اسکن می‌کند معمولاً چیزی می‌خواهد.

- فیلد **قابل ویرایش** می‌ماند تا اگر مهمان QR اتاق اشتباهی را اسکن کرد بتواند اصلاحش کند
- **جایگزین ورود نیست**: کد ملی همچنان لازم است، تطابق کد ملی با اتاق اعتبارسنجی می‌شود و اتاق باید `OCCUPIED` باشد. پس یک QR دستکاری‌شده صرفاً به ورود ناموفق می‌رسد، نه دسترسی به حساب دیگری
- ارقام فارسی و عربی به‌صورت خودکار به ASCII تبدیل می‌شوند؛ هر مقدار نامعتبر نادیده گرفته می‌شود و فیلد خالی می‌ماند

تولید تصویر QR عمداً بیرون از برنامه است — با هر ابزار دلخواهی ساخته می‌شود و وابستگی اضافه‌ای به پروژه تحمیل نمی‌کند.
